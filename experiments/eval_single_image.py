...
def main(modelname, net_config, gan_config, disc_config, datasetSem, datasetGAN, datasetDisc, starting_weights, flag_measure, output_mat, flag_entropy, thresholds, _run):
    ...
    #prediction_array = np.zeros((set_size,h_orig,w_orig))
    # prediction_array = np.zeros((sub_size*4,256,256))
    ...
    for k in range(0,1):
        ...
        for i in range(sub_size):
            ...
            img = cv2.imread('/Users/David/Desktop/Teaser/rgb.jpg')[...,::-1]
            dl_labels =  np.expand_dims(np.load('/Users/David/Desktop/Teaser/prediction_david.npy'),axis=0)
            # img = cv2.imread('/cluster/work/riner/users/haldavid/Teaser/rgb.jpg')
            # dl_labels =  np.expand_dims(np.load('/cluster/work/riner/users/haldavid/Teaser/prediction_david.npy'),axis=0)
        ...
        # matrix_path = os.path.join(output_dir,"synMat"+str(k)+".npy")
        # np.save(matrix_path,synth_images)
        # matrix_path = os.path.join(output_dir,"segmMat"+str(k)+".npy")
        # np.save(matrix_path,segm)
        # matrix_path = os.path.join(output_dir,"rgbMat"+str(k)+".npy")
        # np.save(matrix_path,img_array)
        # matrix_path = os.path.join(output_dir,"simMat"+str(k)+".npy")
        # np.save(matrix_path,simMat)



        # for i in range(sub_size):
        #     # prediction_array[i+kb,...] = cv2.resize(predMat[i,...], (w_orig, h_orig),interpolation=cv2.INTER_NEAREST)
        #     prediction_array[i+kb,...] = predMat[i,...]
        # prediction_array[counter:counter+sub_size,...] = predMat
        # counter += sub_size
