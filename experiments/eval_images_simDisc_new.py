...
def main(modelname, net_config, gan_config, disc_config, datasetSem, datasetGAN, datasetDisc, starting_weights, flag_measure, output_mat, flag_entropy, thresholds, start, _run):
    ...
    with GAN_graph.as_default():
        ...
        if gan_config['type'] == 'cascRef':
            ...
            modelGAN = cGAN_model(GAN_sess,dataset_name='cityscapes_cascGAN',image_size=disc_config['input_image_size'],
                               ...
                               is_training=False, checkpoint=ckp, vgg_checkpoint="/Users/David/Downloads/VGG_Model/imagenet-vgg-verydeep-19.mat")
        else:
    ...
    # base_path = "/Users/David/Downloads/"
    ...
    semseg_path = "/Users/David/Downloads/fishyscapes_deeplab_predictions_newfog_rightorder/fishyscapes_predictions_rightorder"
    out_path = "/Users/David/Desktop/out/here"
    ...
    for k in range(start,(start+2)):
        ...
        for i in range(sub_size):
            ...
            img = cv2.imread(path.join('/Users/David/Downloads/testset', str(i+kb)+'_rgb.png'))
        ...
        for i in range(sub_size):
            ...
            np.save(filename,simMat[i,...])
            filename = path.join(out_path,str(i+kb)+'_synth.npy')
            np.save(filename,synth_images[i,...])
            filename = path.join(out_path,str(i+kb)+'_rgb.npy')
            np.save(filename,img_array[i,...])
